...
describe('강수량 스펙 카드', () => {
  ...
  it('강수량이 2.5 미만일 때 "약한 비"를 렌더링해야 한다', () => {
    const wrapper = mount(PrecipitationSpec, { props: { precipitation: 1.5 } })
    expect(wrapper.text()).toContain('약한 비')
  })

  it('강수량이 7.6 미만일 때 "보통 비"를 렌더링해야 한다', () => {
    const wrapper = mount(PrecipitationSpec, { props: { precipitation: 5.0 } })
    expect(wrapper.text()).toContain('보통 비')
  })

  it('강수량이 7.6 이상일 때 "강한 비"를 렌더링해야 한다', () => {
    const wrapper = mount(PrecipitationSpec, { props: { precipitation: 10.0 } })
