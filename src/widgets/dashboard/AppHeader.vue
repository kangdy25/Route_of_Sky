...
const dashboardToggleLabel = computed(() => (props.isDashboardOpen ? 'Hide dashboard' : 'Show dashboard'))
const logoUrl = '/logo.webp'
</script>

<template>
  <!-- 전역 상단 헤더 컨테이너 (모바일 2열 그리드 / 데스크톱 3단 바) -->
  <header
  ...
  >
    <!-- 네온 하이라이트 구역 -->
    <div
      class="pointer-events-none absolute inset-x-8 top-0 h-px bg-linear-to-r from-transparent via-cyan-200/80 to-transparent shadow-[0_0_16px_rgba(34,211,238,0.8)] sm:inset-x-16"
    />
    <!-- 서비스 브랜드 로고 및 타이틀 -->
    <div class="flex min-w-0 items-center gap-2 sm:gap-3">
      ...
      <h1
        class="truncate bg-linear-to-r from-cyan-100 via-sky-400 to-orange-300 bg-clip-text text-xl font-black text-transparent drop-shadow-[0_0_12px_rgba(34,211,238,0.34)] sm:text-2xl lg:text-3xl"
      >
        Route of Sky
      </h1>
    </div>

    <!-- 지역 선택 셀렉트 및 날씨 동기화 상태 바 -->
    ...
    <!-- 글로벌 인터랙션 액션 버튼 그룹 -->
    <div class="col-start-2 row-start-1 flex justify-end gap-2 sm:gap-3 lg:col-start-auto lg:row-start-auto lg:gap-4">
      <!-- 모바일 대시보드 토글 버튼 -->
      <button
        type="button"
      ...
      >
        <svg class="h-5 w-5 sm:h-6 sm:w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          ></path>
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5h5v5"></path>
          <path
            stroke-linecap="round"
            stroke-linejoin="round"
            stroke-width="2"
