...
import { getCurrentLocalTimeForLocation, getSceneDateFromLocalTime, getSkyPhase, getSunPositionForTime } from './sky'
...
describe('하늘 시간 계산', () => {
  ...
  it('선택 도시의 UTC 오프셋으로 로컬 시간을 변환해야 한다', () => {
    expect(getSceneDateFromLocalTime(16.5, WORLD_LOCATIONS[2]).toISOString()).toBe('2026-06-20T07:30:00.000Z')
    expect(getSceneDateFromLocalTime(16.5, WORLD_LOCATIONS[9]).toISOString()).toBe('2026-06-20T11:00:00.000Z')
  })

  it('현재 UTC 시간을 선택 도시의 24시간 현지 시간으로 변환해야 한다', () => {
    expect(getCurrentLocalTimeForLocation(WORLD_LOCATIONS[2], new Date('2026-06-27T03:15:00.000Z'))).toBe(12.3)
    expect(getCurrentLocalTimeForLocation(WORLD_LOCATIONS[1], new Date('2026-06-27T03:15:00.000Z'))).toBe(23.3)
  })

  it('JulianDate 기준 태양 위치 벡터를 계산해야 한다', () => {
    const result = getSunPositionForTime(JulianDate.fromDate(new Date('2026-06-20T07:30:00.000Z')), new Cartesian3())

    expect(Cartesian3.magnitude(result)).toBeGreaterThan(0)
  })

  it('ICRF 변환 행렬이 없으면 TEME 변환 행렬로 대체해야 한다', () => {
    const icrfSpy = vi.spyOn(Transforms, 'computeIcrfToCentralBodyFixedMatrix').mockReturnValueOnce(undefined)
    const temeSpy = vi.spyOn(Transforms, 'computeTemeToPseudoFixedMatrix')

    getSunPositionForTime(JulianDate.fromDate(new Date('2026-06-20T07:30:00.000Z')), new Cartesian3())
