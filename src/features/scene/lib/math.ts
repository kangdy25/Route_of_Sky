...
/** 두 경계 사이를 부드러운 에르미트 삼차 곡선(S-커브)으로 보간합니다. */
export function smoothstep(edge0: number, edge1: number, value: number): number {
  const span = edge1 - edge0
  // edge0과 edge1이 같을 때 0으로 나누기(NaN)를 방어합니다.
  if (span === 0) return value >= edge1 ? 1 : 0

  const t = clampToUnitInterval((value - edge0) / span)
  return t * t * (3 - 2 * t)
}

/**
 * 각도 래핑을 고려해 가장 짧은 회전 경로(최단 호)로 라디안 값을 보간합니다.
 * 카메라 회전 시 359°에서 1°로 넘어갈 때 반대로 한 바퀴 도는 현상을 방지합니다.
 */
