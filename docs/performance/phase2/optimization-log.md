...
## 2026-08-13 · 정적 성능 예산 CI 기준선

- 가설: 번들·배포 산출물·썸네일은 실제 GPU보다 재현성 높은 지표이므로, 숫자 예산을 CI에서 자동 차단하면 성능 회귀를 병합 전에 발견할 수 있다.
- Before: app entry JS/CSS gzip 86,752/14,947 bytes, `dist/` 13,534,839 bytes, 썸네일 213,019 bytes였지만 이 값을 자동 검증하는 CI 단계는 없었다.
- 목표: JS 90KiB, CSS 18KiB, `dist/` 13.63MiB, 썸네일 250KiB 이하다. 실제 GPU는 환경 의존성이 있어 차단 조건이 아닌 PR 첨부 3회 측정으로 검토한다.
- 롤백 조건: 측정기가 app entry가 아닌 파일을 집계하거나 정상 build를 거짓 실패시키면 게이트를 되돌리고 집계 규칙을 수정한다.

## 2026-08-13 · 정적 성능 예산 CI 적용

- 구현: `perf:budget`과 CI 단계가 build 후 네 가지 바이트 값을 매번 검사한다. 모든 값은 Before와 동일하게 86,752/14,947/13,534,839/213,019 bytes(0 B, 0.0%)였고 각각 예산보다 5,408/3,485/757,252/42,981 bytes 작아 통과했다.
- 검증: `pnpm run build && pnpm run perf:budget`가 네 항목 모두 PASS로 끝났고, CI에는 같은 명령을 추가했다.
- 판단: 정적 회귀를 병합 전에 결정적으로 차단할 수 있다. 이 작업은 기능·번들 변경이 아니므로 0.0%는 기대된 결과다.

## 2026-08-13 · 최종 실제 GPU 재측정·사례 연구

- 방법: 최신 main을 production build한 뒤 `pnpm run perf:gpu -- --label phase2-final-after --runs 3`을 실행했다. headful Chrome 실제 GPU, 1365×768, HTTP 캐시 비활성화, Weather 고정 응답, 프리셋별 20초를 데스크톱·CPU ×4 각각 3회 반복했다. 소프트웨어 렌더러는 측정기에서 거절한다.
- 정적 Before → After: 전체 산출물 14,299,816 → 13,534,839 B(764,977 B, 5.3% 감소), 썸네일 977,995 → 213,019 B(764,976 B, 78.2% 감소)였다. JS gzip은 86,752 → 86,752 B, CSS gzip은 14,950 → 14,947 B이며 정적 예산을 모두 통과했다. 공개 Production에서 썸네일과 Cesium Worker의 1년 immutable 헤더를 확인했다.
- 런타임 Before → After: 데스크톱 FCP/LCP 640 → 732ms(92ms 증가, -14.4%), Viewer 669 → 794.1ms(125.1ms 증가, -18.7%), CPU ×4 FCP/LCP 940 → 1,060ms(120ms 증가, -12.8%), Viewer 1,588 → 2,078.1ms(490.1ms 증가, -30.9%)였다. CPU ×4 Rain/Storm/Snow p95도 282.6/250.0/150.5 → 333.6/298.0/217.3ms로 모두 목표에 미달했다.
- 판단: 초기 로딩·rAF 후보는 이미 코드까지 롤백됐고 최신 main에는 정적 전달 변경만 남는다. 따라서 독립 배치의 런타임 차이를 정적 작업의 인과적 회귀로 단정하거나 개선으로 포장하지 않는다. 미달 수치와 원본 JSON을 그대로 사례 연구에 공개한다.
- 후속·롤백 조건: Chrome trace로 Cesium/Canvas 경쟁을 분리하기 전에는 새 렌더링 후보를 유지하지 않는다. 캐시 헤더 누락·예산 오탐·문서와 원본의 수치 불일치가 나오면 각각 헤더·게이트·문서를 되돌려 재검증한다.
