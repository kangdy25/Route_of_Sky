...
공식 측정은 네트워크·브라우저 캐시를 끈 상태에서 Weather 응답을 고정 모킹한다. 따라서 `2 → 2건`은 Weather 5분 캐시의 실패가 아니라 비교 조건을 일정하게 하는 의도된 결과다. 실제 5분 캐시의 “재방문 API 0건”과 강제 새로고침 우회는 Playwright 통합 테스트에서 검증하며, 캐시 정책과 E2E 근거는 [Phase 1 아카이브 사례 연구](../phase1-case-study.md)에 보존돼 있다. 외부 Google 3D Tiles·실제 Weather API 응답은 p50/p95 관찰 대상이지만 변동만으로 회귀를 판정하지 않는다.
