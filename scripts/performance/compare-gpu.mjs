...
const beforePath = resolve(root, readArgument('--before') ?? 'docs/performance/phase2/runs/real-gpu-before.json')
const afterPath = resolve(root, readArgument('--after') ?? 'docs/performance/phase2/runs/real-gpu-after.json')
const outputPath = resolve(root, readArgument('--output') ?? 'docs/performance/phase2/comparison.md')
...
const metrics = [
  ...
  ['데스크톱 LCP', 'desktop.summary.page.largestContentfulPaintMs.median', 'ms', '기준선 대비 개선'],
  ...
  ['데스크톱 Event Timing p95', 'desktop.summary.page.eventTimingP95Ms.median', 'ms', '기준선 대비 개선'],
  ...
  ['API 네트워크 요청', 'desktop.summary.api.networkRequestCount.median', 'count', '캐시 시나리오 0건'],
  ['API 캐시 반영', 'desktop.summary.page.weatherCacheHitMs.median', 'ms', 'p95 100ms 이하'],
  ['리소스 전송량', 'desktop.summary.page.resourceTransferBytes.median', 'bytes', '기준선 대비 개선'],
]
